...
# interpolate the value SLIQ value on the melt curve that is at the value of the critical point
sc_on_melt_curve = np.interp(critical_point, melt_curve['T'], melt_curve['SLIQ'])
...
for ax in axs:
    ...
    ax.plot(
        np.arange(sc_on_melt_curve, 30000, 10),
        [critical_point] * len(np.arange(sc_on_melt_curve, 30000, 10)),
        linewidth=2.0,
        color='k',
        linestyle="dotted"
    )
    ...
    ax.annotate(
        'S + L',
        xy=(3800, 6000),  # theta, radius
        xytext=(0.35, 0.60),  # fraction, fraction
    ...
    # annotate the solid-vapor mixed region
    ax.text(
        0.50, 0.08, "S + V", transform=ax.transAxes, ha='center', va="center", fontsize=18
    )
    # annotate the supercritical region
    ax.text(
        0.50, 0.70, "SC", transform=ax.transAxes, ha='center', va="center", fontsize=18
    )
    # annotate the pure liquid region
    ax.text(
        0.25, 0.40, "L", transform=ax.transAxes, ha='center', va="center", fontsize=18
    )
    # annotate the pure vapor region
    ax.text(
        0.80, 0.40, "V", transform=ax.transAxes, ha='center', va="center", fontsize=18
    )
    # annotate the solid region
    ax.text(
...
for index, run in enumerate(runs):
    ...
    axs[index].text(0.80, 0.90, f"{len(disk_particles)} particles", transform=axs[index].transAxes,
