...
class GiantImpactReport:
    ...
    def generate_report(self, particles: pd.DataFrame, planet_mass_normalizer: float, disk_mass_normalizer: float,
        ...
        d = {
            ...
            r"$N_{sil,tar}$": len(particles[particles['tag'] == 0]),
            r"$N_{iron,tar}$": len(particles[particles['tag'] == 1]),
            r"$N_{sil,imp}$": len(particles[particles['tag'] == 2]),
            r"$N_{iron,imp}$": len(particles[particles['tag'] == 3]),
